...
const NewProductClient = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    // Filter out empty image/video entries before submitting
    const payload = {
      ...productData,
      images: productData.images.filter((img) => img.src.trim() !== ""),
      videos: productData.videos.filter((vid) => vid.src.trim() !== ""),
      compareAtPrice:
        productData.compareAtPrice === 0 ? null : productData.compareAtPrice, // Ensure null if 0
    };
    ...
    try {
      ...
      Swal.fire("Error", `Failed to create product: ${err.message}`, "error");
    } finally {
      setIsSubmitting(false);
    }
  };
  ...
  return (
    <motion.div
      ...
      <h2 className='text-center text-2xl md:text-3xl font-semibold text-gray-800 mb-6 pb-4'>
        Add New Product
      </h2>
      ...
      <form onSubmit={handleSubmit} className='space-y-8'>
        ...
        <section className='p-4 ring ring-gray-200 rounded-md'>
          ...
          {productData.variants.map((variant, index) => (
            <div
              key={index}
              className='grid grid-cols-1 md:grid-cols-4 gap-2 mb-2 p-3 border rounded-md'>
              <input
                type='text'
                placeholder='Color'
                value={variant.color}
